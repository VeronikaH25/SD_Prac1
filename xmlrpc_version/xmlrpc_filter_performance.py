...
def send_requests(host, num_requests, result_dict, idx): #peticiones al InsultFilter
    ...
    for i in range(num_requests):
        proxy.submit_text("you are a stupid idiot")
        if (i + 1) % 10 == 0:
            print(f"[{host}] Sent {i+1} at {time.time() - start:.2f}s")
    elapsed = time.time() - start #tiempo de 10 peticiones
    result_dict[idx] = (host, num_requests, elapsed) #tiempo total
...
def run_parallel_test(hosts, total_requests):
    manager = multiprocessing.Manager() #ejecucion en paralelo
    ...
    for i, host in enumerate(hosts):
        p = multiprocessing.Process(
            ...
            args=(host, requests_per_host, result_dict, i)
        )
        p.start()
        processes.append(p)
